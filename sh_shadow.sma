...
// VARIABLES
new gHeroName[]="Shadow the Hedgehog"
...
#if SEND_COOLDOWN
new Float:ShadowUsedTime[SH_MAXSLOTS+1]
#endif
//--------------------------------------------------------
public plugin_init()
{
	...
	shCreateHero(gHeroName, "Hero or Villain?", "Activate for Godmode + Chaos Blast", true, "shadow_level" )
...
//----------------------------------------------------------------------------------------------
#if SEND_COOLDOWN
public sendShadowCooldown(id)
{
	new cooldown
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_cvar_num("shadow_cooldown") - get_gametime() + ShadowUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
public shadow_kd()
{
	...
	#if SEND_COOLDOWN
	ShadowUsedTime[id] = get_gametime()
	#endif
...
public shadow_chaosblast(parm[])
{
	...
	for ( new i = 1; i <= SH_MAXSLOTS; i++) {
		...
		if (distance < 800)
			shExtraDamage(i, id, 1200 - distance, "Chaos Blast" )
	}
}
//--------------------------------------------------------
