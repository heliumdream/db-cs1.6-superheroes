...
#include <superheromod>
	...
	//b / relevant cooldown power Herolist	Blink, Bomberman, Dazzler, Electro, Forge, Hulk, Ken, Longshot, Shadowcat, Super Saiyan Gohan, Yoda, Yuna, GokuKT, Sandman, Frieza, Subzero, Super Vegeta - GalitGun, Shadow the Hedgehog, Phasing, Danimoth, Neo Reloaded, Naruto Uzumaki, Secretary of Defense, Storm, Exodus, Quicksilver

	//new BishopForward, ShadowcatForward, SnakeForward, BlinkForward, ElectroForward, JubileeForward

	//new BishopForward, BlinkForward, ElectroForward, ShadowcatForward

	new BishopForward, BlinkForward, BombermanForward, DazzlerForward, ElectroForward, ForgeForward
	new HulkForward, KenForward, LongshotForward, ShadowcatForward, SSGohanForward
	new YodaForward, YunaForward, GokuKTForward, SandmanForward, FriezaForward
	new SubzeroForward, SVegetaForward, ShadowForward, PhasingForward, DanimothForward
	new NeoForward, NarutoForward, SecretaryForward, StormForward, ExodusForward, QuicksilverForward

	new BishopEnergy[SH_MAXSLOTS+1]
	new BlinkCooldown[SH_MAXSLOTS+1]
	new BombermanCooldown[SH_MAXSLOTS+1]
	new DazzlerCooldown[SH_MAXSLOTS+1]
	new ElectroCooldown[SH_MAXSLOTS+1]
	new ForgeCooldown[SH_MAXSLOTS+1]
	new HulkCooldown[SH_MAXSLOTS+1]
	new KenCooldown[SH_MAXSLOTS+1]
	new LongshotCooldown[SH_MAXSLOTS+1]
	new ShadowcatCooldown[SH_MAXSLOTS+1]
	new SSGohanCooldown[SH_MAXSLOTS+1]
	new YodaCooldown[SH_MAXSLOTS+1]
	new YunaCooldown[SH_MAXSLOTS+1]
	...
	new	SVegetaCooldown[SH_MAXSLOTS+1]
	new	ShadowCooldown[SH_MAXSLOTS+1]
	new	PhasingCooldown[SH_MAXSLOTS+1]
	new	DanimothCooldown[SH_MAXSLOTS+1]
	new NeoCooldown[SH_MAXSLOTS+1]
	new	NarutoCooldown[SH_MAXSLOTS+1]
	new	SecretaryCooldown[SH_MAXSLOTS+1]
	new	StormCooldown[SH_MAXSLOTS+1]
	new	ExodusCooldown[SH_MAXSLOTS+1]
	new	QuicksilverCooldown[SH_MAXSLOTS+1]
...
public plugin_init()
{
	...
	sh_anubisdmg_check = register_cvar("anubis_showdamage", "1")
	...
	register_logevent("vip_Escaped", 6, "3=VIP_Escaped")
		...
		BishopForward = CreateMultiForward("sendAbsorbedDamage", ET_CONTINUE, FP_CELL)
		BlinkForward = CreateMultiForward("sendBlinkCooldown", ET_CONTINUE, FP_CELL)
		BombermanForward = CreateMultiForward("sendBombermanCooldown", ET_CONTINUE, FP_CELL)
		DazzlerForward = CreateMultiForward("sendDazzlerCooldown", ET_CONTINUE, FP_CELL)
		ElectroForward = CreateMultiForward("sendElectroCooldown", ET_CONTINUE, FP_CELL)
		ForgeForward = CreateMultiForward("sendForgeCooldown", ET_CONTINUE, FP_CELL)
		HulkForward = CreateMultiForward("sendHulkCooldown", ET_CONTINUE, FP_CELL)
		KenForward = CreateMultiForward("sendKenCooldown", ET_CONTINUE, FP_CELL)
		LongshotForward = CreateMultiForward("sendLongshotCooldown", ET_CONTINUE, FP_CELL)
		ShadowcatForward = CreateMultiForward("sendShadowcatCooldown", ET_CONTINUE, FP_CELL)
		SSGohanForward = CreateMultiForward("sendSSGohanCooldown", ET_CONTINUE, FP_CELL)
		YodaForward = CreateMultiForward("sendYodaCooldown", ET_CONTINUE, FP_CELL)
		YunaForward = CreateMultiForward("sendYunanCooldown", ET_CONTINUE, FP_CELL)
		...
		SubzeroForward = CreateMultiForward("sendSubzeroCooldown", ET_CONTINUE, FP_CELL)
		SVegetaForward = CreateMultiForward("sendSVegetaCooldown", ET_CONTINUE, FP_CELL)
		ShadowForward = CreateMultiForward("sendShadowCooldown", ET_CONTINUE, FP_CELL)
		PhasingForward = CreateMultiForward("sendPhasingCooldown", ET_CONTINUE, FP_CELL)
		...
		NarutoForward = CreateMultiForward("sendNarutoCooldown", ET_CONTINUE, FP_CELL)
		SecretaryForward = CreateMultiForward("sendSecretaryCooldown", ET_CONTINUE, FP_CELL)
		StormForward = CreateMultiForward("sendStormCooldown", ET_CONTINUE, FP_CELL)
		ExodusForward = CreateMultiForward("sendExodusCooldown", ET_CONTINUE, FP_CELL)
		QuicksilverForward = CreateMultiForward("sendQuicksilverCooldown", ET_CONTINUE, FP_CELL)
...
public plugin_precache()
{
	...
	//precache_sound("shmod/protoman_active.wav")
	//precache_sound("shmod/protoman_deflect.wav")

	// Protoman superheromod extension
	gSpriteShield = precache_model("sprites/shmod/protoman_powerup.spr")	
...
public _sh_create_hero()
{
	...
	if ( equal(pHero, "Anubis") ) gAnubisHero = "Anubis"	
...
public cl_clearpowers(id)
{
	...
	if ( !get_pcvar_num(sh_alivedrop) && is_user_alive(id) && !(get_user_flags(id)&ADMIN_IMMUNITY) ) {
...
public roundStartDelay()
{
	...
	for ( new x = 1; x <= gServersMaxPlayers; x++ ) {
		...
		//displayPowers delayed on roundStart, allows players to grok xp message before it gets removed for active powers info / cooldown system
		//new args[2]
		//args[0]=x
		//args[1]=true
		//set_task(3.0, "displayPowers", x, args, 2)
...
displayPowers(id, bool:setThePowers)
{
	...
	//gRoundFreeze
	if ( playerLevel < gNumLevels && gRoundFreeze) {
		formatex(message, charsmax(message), "LVL:%d/%d XP:(%d/%d)", playerLevel, gNumLevels, gPlayerXP[id], gXPLevel[playerLevel+1])
	}
	else if (gRoundFreeze) {
		formatex(message, charsmax(message), "LVL:%d/%d XP:(%d)", playerLevel, gNumLevels, gPlayerXP[id])
	}
	//else if (gPlayerUltimateUsed[id]) {
	//	formatex(message, charsmax(message), "Cooldowns:")
	//}
	//else formatex(message, charsmax(message), "Heroes (gPUU[id]:%d):", gPlayerUltimateUsed[id])
	else if (ShadowcatCooldown[id] > 0 || BlinkCooldown[id] > 0 || ElectroCooldown[id] > 0 || SandmanCooldown[id] > 0  || BombermanCooldown[id] > 0 || DanimothCooldown[id] > 0 || DazzlerCooldown[id] > 0 || ExodusCooldown[id] > 0 || ForgeCooldown[id] > 0 || FriezaCooldown[id] > 0  || GokuKTCooldown[id] > 0) {
		formatex(message, charsmax(message), "Cooldowns:")
	}
	else if (HulkCooldown[id] > 0 || KenCooldown[id] > 0 || LongshotCooldown[id] > 0 || NarutoCooldown[id] > 0 || NeoCooldown[id] > 0 || PhasingCooldown[id] > 0 || QuicksilverCooldown[id] > 0 || SecretaryCooldown[id] > 0 || ShadowCooldown[id] > 0  || SSGohanCooldown[id] > 0  || StormCooldown[id] > 0  || SubzeroCooldown[id] > 0  || SVegetaCooldown[id] > 0 || YodaCooldown[id] > 0 || YunaCooldown[id] > 0) {
		formatex(message, charsmax(message), "Cooldowns:")
	}
	else formatex(message, charsmax(message), "Heroes:")
			...
			if ( gSuperHeros[heroIndex][requiresKeys] ) {
				...
				if (count <= 9) {
						...
						if ( equal(gSuperHeros[heroIndex], "Bishop") )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], BishopEnergy[id] )
						else if ( equal(gSuperHeros[heroIndex], "Shadowcat") && ShadowcatCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], ShadowcatCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Blink") && BlinkCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], BlinkCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Electro") && ElectroCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], ElectroCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Sandman") && SandmanCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], SandmanCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Bomberman") && BombermanCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], BombermanCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Danimoth X") && DanimothCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], DanimothCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Dazzler") && DazzlerCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], DazzlerCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Exodus") && ExodusCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], ExodusCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Forge") && ForgeCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], ForgeCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Frieza") && FriezaCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], FriezaCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Goku's Kaioken") && GokuKTCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], GokuKTCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "The Hulk") && HulkCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], HulkCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Ken Masters") && KenCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], KenCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Longshot") && LongshotCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], LongshotCooldown[id] )		
						else if ( equal(gSuperHeros[heroIndex], "Naruto Uzumaki") && NarutoCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], NarutoCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Neo Reloaded") && NeoCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], NeoCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Phasing (Matrix Twins)") && PhasingCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], PhasingCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Quicksilver") && QuicksilverCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], QuicksilverCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Secretary of Defense") && SecretaryCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], SecretaryCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Shadow the Hedgehog") && ShadowCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], ShadowCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Super Saiyan Gohan") && SSGohanCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], SSGohanCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Storm") && StormCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], StormCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Sub-Zero") && SubzeroCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], SubzeroCooldown[id] )
						else if ( equal(gSuperHeros[heroIndex], "Super Vegeta") && SVegetaCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], SVegetaCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Yoda") && YodaCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], YodaCooldown[id] )	
						else if ( equal(gSuperHeros[heroIndex], "Yuna") && YunaCooldown[id] > 0 )
							format( ShowPowerInfo, 32, "%s(%i)", gSuperHeros[heroIndex], YunaCooldown[id] )									
						else 
							ShowPowerInfo = ""
						
						if ( message[0] != '^0') add(message, charsmax(message), " ")

						if (ShadowcatCooldown[id] > 0 || BlinkCooldown[id] > 0 || ElectroCooldown[id] > 0 || SandmanCooldown[id] > 0  || BombermanCooldown[id] > 0 || DanimothCooldown[id] > 0 || DazzlerCooldown[id] > 0 || ExodusCooldown[id] > 0 || ForgeCooldown[id] > 0 || FriezaCooldown[id] > 0  || GokuKTCooldown[id] > 0) {
							formatex(temp, charsmax(temp), "%s", ShowPowerInfo)
						}
						else if (HulkCooldown[id] > 0 || KenCooldown[id] > 0 || LongshotCooldown[id] > 0 || NarutoCooldown[id] > 0 || NeoCooldown[id] > 0 || PhasingCooldown[id] > 0 || QuicksilverCooldown[id] > 0 || SecretaryCooldown[id] > 0 || ShadowCooldown[id] > 0  || SSGohanCooldown[id] > 0  || StormCooldown[id] > 0  || SubzeroCooldown[id] > 0  || SVegetaCooldown[id] > 0 || YodaCooldown[id] > 0 || YunaCooldown[id] > 0) {
							formatex(temp, charsmax(temp), "%s", ShowPowerInfo)
						}
...
public _sh_extra_damage()
{
	...
	get_user_info(victim,"ActiveProto",soutput,7)
	output = str_to_num(soutput)

	if( newHealth < 1 && output == 1 ) { 
		// check for wpnDescription != "m4a1"...p90...etc...>
		// this check doesnt have gWpnindex like protoman...only broken for old style gun damage based on sh damage not gun damage based on multiplier

		//if ( equal(wpnDescription, "knife") ) client_print(attacker, print_chat,"proto lethal check wpn=knife")
		//client_print(attacker, print_chat,"proto lethal check wpn=%s", wpnDescription)

		new randNum = random_num(0, 100)
		new protoChance = floatround(get_cvar_float("protoman_chance") * 100)
		//client_print(attacker, print_chat,"[SH](Protoman): protoChance=%d, randNum=%d", protoChance, randNum)
		if (protoChance >= randNum)
		{
			...
			if (attacker == victim) {
			    ...
			    new message[128]
			    format(message, 127, "Protoman deflected your own fatal %s projectile!", wpnDescription)
			    set_hudmessage(200,0,0,-1.0,0.36,0,0.25,2.0,0.0,0.0,-1)
			    show_hudmessage(attacker, message)
			}
			else
			{
			    new message[128]
			    format(message, 127, "Protoman deflected the fatal %s projectile!", wpnDescription )
			    set_hudmessage(0,120,255,-1.0,0.38,0,0.25,2.0,0.0,0.0,-1)
			    show_hudmessage(victim, message)

			    //client_print(id, print_chat, "debug pev-attacker: %d", attacker)						   
           
			    format(message, 127, "Protoman deflected your fatal %s projectile!", wpnDescription )
			    set_hudmessage(0,120,255,-1.0,0.38,0,0.25,2.0,0.0,0.0,-1)
			    show_hudmessage(attacker, message)

			    //could add glow or sprite effect 
			    //sh_set_rendering(attacker, 8, 8, 8, alpha, kRenderFxGlowShell, kRenderTransAlpha)
			}
			//may need to precache inside superheromod...maybe picked up from protoman
			emit_sound(victim, CHAN_AUTO, "shmod/protoman_deflect.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)

			new args[1]
			args[0]=victim
			set_task(0.05,"protoman_shield_effect",victim+1,args,1,"a",10)
		}
	}
	...
	if ( newHealth < 1 ) {
		...
		if ( gHasAnubis[attacker] && get_pcvar_num(sh_anubisdmg_check) && victim != attacker ) {
			set_hudmessage(0, 100, 200, -1.0, 0.51, 2, 0.1, 2.0, 0.02, 0.02, -1)
			ShowSyncHudMsg(attacker, gMsgSync1, "SH: %d %s - Lethal!", damage, wpnDescription)
		...
		if ( gHasAnubis[victim] && get_pcvar_num(sh_anubisdmg_check) ) {
			set_hudmessage(200, 0, 0, -1.0, 0.44, 2, 0.1, 2.0, 0.02, 0.02, -1)
			ShowSyncHudMsg(victim, gMsgSync2, "SH: %d %s - Lethal!", damage, wpnDescription)
		...
		if ( gHasAnubis[victim] && get_pcvar_num(sh_anubisdmg_check) ) {
			set_hudmessage(200, 0, 0, -1.0, 0.44, 2, 0.1, 2.0, 0.02, 0.02, -1)
			ShowSyncHudMsg(victim, gMsgSync2, "SH: %d %s", damage, wpnDescription)
			//sh_chat_message(victim, -1, "Anubis SH damage %d - %s", damage, wpnDescription)
		}

		if ( gHasAnubis[attacker] && get_pcvar_num(sh_anubisdmg_check) && victim != attacker ) {
			set_hudmessage(0, 100, 200, -1.0, 0.51, 2, 0.1, 2.0, 0.02, 0.02, -1)
			ShowSyncHudMsg(attacker, gMsgSync1, "SH: %d %s", damage, wpnDescription)
			//sh_chat_message(attacker, -1, "Anubis SH damage %d - %s ", damage, wpnDescription)
		}		
...
//	}
	...
	ExecuteForward(BombermanForward, functionReturn, id)
	if ( BombermanCooldown[id] != functionReturn )
	{
		BombermanCooldown[id] = functionReturn
		flag = true
	}
	...
	ExecuteForward(DazzlerForward, functionReturn, id)
	if ( DazzlerCooldown[id] != functionReturn )
	{
		DazzlerCooldown[id] = functionReturn
		flag = true
	}

	ExecuteForward(ExodusForward, functionReturn, id)
	if ( ExodusCooldown[id] != functionReturn )
	{
		ExodusCooldown[id] = functionReturn
		flag = true
	}

	ExecuteForward(ForgeForward, functionReturn, id)
	if ( ForgeCooldown[id] != functionReturn )
	{
		ForgeCooldown[id] = functionReturn
		flag = true
	}					
	...
	ExecuteForward(HulkForward, functionReturn, id)
	if ( HulkCooldown[id] != functionReturn )
	{
		HulkCooldown[id] = functionReturn
		flag = true
	}			

	ExecuteForward(KenForward, functionReturn, id)
	if ( KenCooldown[id] != functionReturn )
	{
		KenCooldown[id] = functionReturn
		flag = true
	}		

	ExecuteForward(LongshotForward, functionReturn, id)
	if ( LongshotCooldown[id] != functionReturn )
	{
		LongshotCooldown[id] = functionReturn
		flag = true
	}			
	
	ExecuteForward(NarutoForward, functionReturn, id)
	if ( NarutoCooldown[id] != functionReturn )
	{
		NarutoCooldown[id] = functionReturn
		flag = true
	}		
	...
	ExecuteForward(PhasingForward, functionReturn, id)
	if ( PhasingCooldown[id] != functionReturn )
	{
		PhasingCooldown[id] = functionReturn
		flag = true
	}				

	ExecuteForward(QuicksilverForward, functionReturn, id)
	if ( QuicksilverCooldown[id] != functionReturn )
	{
		QuicksilverCooldown[id] = functionReturn
		flag = true
	}	

	ExecuteForward(SecretaryForward, functionReturn, id)
	if ( SecretaryCooldown[id] != functionReturn )
	{
		SecretaryCooldown[id] = functionReturn
		flag = true
	}		

	ExecuteForward(ShadowForward, functionReturn, id)
	if ( ShadowCooldown[id] != functionReturn )
	{
		ShadowCooldown[id] = functionReturn
		flag = true
	}			

	ExecuteForward(SSGohanForward, functionReturn, id)
	if ( SSGohanCooldown[id] != functionReturn )
	{
		SSGohanCooldown[id] = functionReturn
		flag = true
	}					

	ExecuteForward(StormForward, functionReturn, id)
	if ( StormCooldown[id] != functionReturn )
	{
		StormCooldown[id] = functionReturn
		flag = true
	}		
	...
	ExecuteForward(SVegetaForward, functionReturn, id)
	if ( SVegetaCooldown[id] != functionReturn )
	{
		SVegetaCooldown[id] = functionReturn
		flag = true
	}			
	...
	ExecuteForward(YunaForward, functionReturn, id)
	if ( YunaCooldown[id] != functionReturn )
	{
		YunaCooldown[id] = functionReturn
		flag = true
	}		
...
#endif
//---------------------------------------------------------------------------------------------
public protoman_shield_effect(parm[])
{
    ...
    for (new i = 0; i < pnum; i++) {
        ...
        write_short(gSpriteShield)    // sprite index
...
public cl_say(id)
{
	...
	if ( equali(said[pos], "superherohelp") || equali(said[pos], "help") ) {
		...
		if ( !get_pcvar_num(sh_alivedrop) && is_user_alive(id) ) {
			...
			// admins can still drop powers while alive.
			if ( (get_user_flags(id)&ADMIN_IMMUNITY) ) {
				clearAllPowers(id, true)
				chatMessage(id, _, "All your powers have been cleared admin - other player are not allowed to drop heroes while alive")			
			}
			else {
				chatMessage(id, _, "You are not allowed to drop heroes while alive")
			}
...
dropPower(id, const said[])
{
	...
	else if ( !get_pcvar_num(sh_alivedrop) && is_user_alive(id) && (get_user_flags(id)&ADMIN_IMMUNITY) ) {
		chatMessage(id, _, "Only admins are allowed to drop heroes while alive")
	}
